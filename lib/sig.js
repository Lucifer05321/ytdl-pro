...
exports.extractFunctions = body => {
  ...
  const extractDecipher = () => {
    const patterns = [
      { start: `a.set("alr","yes");c&&(c=`, end: `(decodeURIC` },
      { start: `c&&(c=`, end: `(decodeURIComponent(c))` },
      { start: `b=a.split("")`, end: null }
    ];
    
    let functionName = null;
    for (const pattern of patterns) {
      if (pattern.end) {
        functionName = utils.between(body, pattern.start, pattern.end);
      } else {
        const match = body.match(/([a-zA-Z0-9$]+)=function\(a\)\{a=a\.split\(""\)/);
        if (match) functionName = match[1];
      }
      if (functionName && functionName.length) break;
    }
  ...
  const extractNCode = () => {
    ...
    if (!functionName || !functionName.length) {
      const match = body.match(/\.get\("n"\)\)&&\(b=([a-zA-Z0-9$]+)(?:\[(\d+)\])?\(b\)/);
      if (match) functionName = match[2] ? `${match[1]}[${match[2]}]` : match[1];
    }
    if (functionName && functionName.includes('[')) {
      const varName = functionName.split('[')[0];
      const temp = utils.between(body, `var ${varName}=[`, `]`);
      if (temp) functionName = temp;
    }
    if (functionName && functionName.length) {
      const functionStart = `${functionName}=function(a)`;
      const ndx = body.indexOf(functionStart);
